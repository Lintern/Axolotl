...
mod tests {
    ...
    async fn download_progress_speed_drops_without_retaining_the_peak() {
		...
		let mib = 1024 * 1024;
		{
			...
			state.initialized_from_store = true;
			state.job.active_downloads.insert("file".to_string(), ActiveDownloadState {
				name: "file".to_string(),
				url: String::new(),
				source: String::new(),
				...
				speed_sample_started_at: Utc::now() - chrono::TimeDelta::seconds(1),
				speed_sample_started_bytes: 40 * mib,
			});
		}
		reporter.record_download_progress("file", 42 * mib, 100 * mib).await.unwrap();
		let state = reporter.state.lock().await;
		let speed = state.job.active_downloads["file"].speed_bytes_per_second.unwrap();
